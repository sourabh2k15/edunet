...
<head>
   <link rel="icon" type="image/png" href="images/favicon.ico">
	 <?php 
	    ...
	    echo "<link rel='stylesheet' type='text/css' href='css/".$_COOKIE['browser']."/quora.css'>";
	 ?>
  ...
  <div class='papercontainer'>
    <div id='menu'> 
      <div id='userbar' class='userbar'>
        <span id='username'><a href="profile.php?scholar=<?php echo trim($user_scholar)?>" class='usertitle'><?php echo $user_name?></a></span>
      </div>
      <div id='spancon'>
    ...
    <?php
      $ref = 'questions/reference.xml';
      $doc = new DOMDocument();
      $doc->load($ref);
      $count = $doc->getElementsByTagName('reference')->item(0)->getAttribute('id');
      $count++;
         for($i =0 ;$i< $count; $i++){
		 	$filename= 'questions/question'.$i.'.xml';
		 	if(file_exists($filename)){
		 		$doc->load($filename);
		 	    $id = $doc->getElementsByTagName('qa')->item(0)->getAttribute('id');
		 	    $question = $doc->getElementsByTagName('question')->item(0)->nodeValue;
		 	    echo $id.' '.$question.'<br/>';
		 	    }
		 }
